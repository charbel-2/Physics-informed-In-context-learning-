...
with open('learned_params_IDSIA_2_5_6__again.json', 'r') as f:
...
stitched_pred_model = stitch_predictions(predictions)
stitched_y_test = stitch_predictions(y_test_np)


print('--- Metrics on Stitched Time Series (each time step unique) ---')
j = 2
mse_val = mean_squared_error(stitched_y_test[start:, j], stitched_pred_model[start:, j])
rmse_val = root_mean_squared_error(stitched_y_test[start:, j], stitched_pred_model[start:, j])
mae_val = mean_absolute_error(stitched_y_test[start:, j], stitched_pred_model[start:, j])
r2_val  = r2_score(stitched_y_test[start:, j], stitched_pred_model[start:, j])
print(f"Physics-Driven - Z Axis: MSE: {mse_val:.4f}, RMSE: {rmse_val:.4f}, MAE: {mae_val:.4f}, R²: {r2_val:.4f}")
print("-----------------------------------------------------------")


plt.figure(figsize=(14,5))
plt.plot(stitched_y_test[start:, 2], '--', label='Ground Truth')
plt.plot(stitched_pred_model[start:, 2], label='Physics-Based')
