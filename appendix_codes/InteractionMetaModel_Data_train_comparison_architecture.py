...
from DataDriven_interaction_model import EnhancedTransformerData, LSTMInteractionModel, DecoderOnlyInteractionModel, TCNInteractionModel, DeepSetInteractionModel
SEED = 42
...
df4 = pd.read_csv(csv_file_path_table)
...
data4 = df4[relevant_columns].dropna()
data_outputs4 = df4[relevant_outputs].dropna()
...
outputs = [data_outputs,data_outputs2, data_outputs3, data_outputs4]
...
try:
    ...
    # Creation of the models to comparre with the encoder-decoder transformer 
    models = {
    ...
    for name, model in models.items():
        ...
        wandb.init(project="meta-Franka-ablation", name=f"Data_{name}_run", reinit=True)

        criterion = nn.MSELoss()
        optimizer = torch.optim.AdamW(model.parameters(), lr=0.0001)
        @torch.no_grad()
        # Function for estimation of the validation loss
        def estimate_loss(model,name, loaders):
            model.eval()
            total_loss = 0.0
            loader_iters = [iter(loader) for loader in loaders]  
            num_batches = [len(loader) for loader in loaders]

            for selected_idx in range(len(loaders)):      
                loss = 0.0
                for _ in range(num_batches[selected_idx]):        
                    ...
                    if name == "Transformer":
                        ...
                        output = model(X_batch_norm[:,:,:-3],X_batch_norm[:,:,-3:], X_decoder_batch_norm)
        ...
        while time_t < time_threshold:
            ...
            if epoch % 100 == 0:
                loss_val = estimate_loss(model,name, val_loaders)
                print(f"\n{epoch=} {loss_val=:.4f}\n")

                if loss_val < best_val_loss:
                    wandb.log({"iter_save": epoch})
                    best_val_loss = loss_val
                    torch.save(model.state_dict(), f'comparison_Data_architecture_ckpts/Interaction_metamodel_data_{name}_test_right_all4.pth')
                    print("Model Saved!")
            if name == "Transformer":
                if epoch==epoch_num:
                    # save the threshold time computed for the transformer training, and use that for the other architectures
                    time_threshold = time.time()-start
                    break
            else:
                time_t = time.time()-start
        epoch_param.append([epoch,num_params])
    print(epoch_param)
