...
try:
    ...
    for lam_phy in np.arange(0,0.8,0.05):
        ...
        wandb.init(project="meta-Franka-ablation", name=f"Phys_{lam_phy}_run", reinit=True)
        ...
        criterion = PhysicsBasedLoss(lambda_phy=lam_phy, initial_params=initial_params, lower_bounds=lower_bounds).to(device) #0.5 working well
        ...
        while epoch < epoch_num and loss > loss_threshold:
            ...
            if epoch % 200 == 0:
                ...
                if loss_val < best_val_loss:
                    ...
                    torch.save(model.state_dict(), f'fine_tune_phy_Lam_ckpts/Interaction_metamodel_physics_{lam_phy}_test_right.pth')
                    print("Model Saved!")
                learned_params = {
                ...
                with open(f'fine_tune_phy_Lam_ckpts/{lam_phy}learned_params_IDSIA_2_5_6__again.json', 'w') as f:
